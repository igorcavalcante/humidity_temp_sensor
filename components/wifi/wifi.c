/*
    Based on espressif wifi get started example
*/
#include "wifi.h"
#include "esp_event.h"
#include "esp_log.h"
...
void clearWifiEventBits() {
  xEventGroupClearBits(conn_state, WIFI_FAIL_BIT | WIFI_CONNECTED_BIT);
}
...
static void event_handler(void *arg, esp_event_base_t event_base, int32_t event_id,
  ...
  if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
    ...
    clearWifiEventBits();
    xEventGroupSetBits(conn_state, WIFI_FAIL_BIT);
    ...
    s_retry_num = 0;
    clearWifiEventBits();
    xEventGroupSetBits(conn_state, WIFI_CONNECTED_BIT);
  }
}
...
void wifi_init(void) {
  ...
  EventBits_t bits = xEventGroupWaitBits(conn_state, WIFI_CONNECTED_BIT | WIFI_FAIL_BIT,
